...
var discordBotToken = 'YOURTOKENHERE';	//DISCORD TOKEN
var prefix = 'groles_';	//BOT PREFIX
...
client.on('presenceUpdate', (oldPresence, newPresence) => {
	...
	if(oldPresence == 'undefined' && newPresence == 'undefined') {return}
	if(oldPresence.activities == 'undefined' && newPresence.activities == 'undefined') {return}
	
	//REMOVE BOTS UPDATE
	for (var ii in blackListUsers){
	if(oldPresence.userID.includes(blackListUsers[ii].toLowerCase())) {return}}
	...
	if(oldActivities == newActivities) {return};
	...
	console.log('Atividade antiga: '+oldActivities+'\nNova atividade: '+newActivities+'\nUser: '+newPresence.member.displayName);
      ...
      if (oldGame) {
		newPresence.member.roles.remove(roleGame[oldGame].id).catch(e => {console.error(e)});
